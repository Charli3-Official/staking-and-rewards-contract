...
export async function sendStake({ value, provider_address, locked_until }) {
  ...
  const providerPubKeyHash =
    lucid.utils.getAddressDetails(provider_address).paymentCredential.hash;
...
export async function retireStake({ inUtxo, provider_addr }) {
  ...
  console.log(
    `Provider UTXO: ${providerUtxo.txHash}#${providerUtxo.outputIndex}`,
  );
...
async function findActiveStakingUtxo(
  validatorAddress,
  lucid,
  providerPubKeyHash,
  currentTime,
  ...
  return findStakingUtxo(validatorAddress, lucid, ({ datum }) => {
    return (
      datum.provider_key === providerPubKeyHash &&
...
async function findRetiringStakingUtxo(
  validatorAddress,
  lucid,
  providerPubKeyHash,
  currentTime,
  ...
  return findStakingUtxo(validatorAddress, lucid, ({ utxo, datum }) => {
    ...
    const tokenUnit =
      policyId === "" && assetName === "" ? "lovelace" : policyId + assetName;
    const hasTokens =
      utxo.assets[tokenUnit] && BigInt(utxo.assets[tokenUnit]) > 0n;
    return (
      datum.provider_key === providerPubKeyHash &&
      datum.locked_until <= BigInt(currentTime) &&
...
async function findUtxoByState(
  validatorAddress,
  lucid,
  providerPubKeyHash,
  state,
...
async function requestCertificateFromIssuer(
  ...
  console.log(
    `Certificate received (expires: ${new Date(Number(certificate.expiresAt)).toISOString()})`,
  );

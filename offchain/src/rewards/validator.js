...
export function rewardValidator(operator_vkey, owner_pubkey_hash) {
    const validator = PLUTUS_BLUEPRINT.validators.find(({ title }) => title == "reward.reward")
    return {
        type: "PlutusV2",
        script: applyParamsToScript(validator.compiledCode, [toHex(operator_vkey), owner_pubkey_hash]),
    };
}
...
export async function placeReward({ value, owner_addr }, validator) {
    ...
    const tx = await lucid.newTx().payToContract(validatorAddr, Data.void(), value).complete()
...
export async function claimReward({ provider_addr }, validator) {
    ...
    const inputRewardUtxo = rewardUtxoWithDatum.filter((u) => u.assets.lovelace >= BigInt(2000000))[0]
    ...
    let outRewardUtxoAsset = { ...inputRewardUtxo.assets, lovelace: (totLovelaceRewardUtxo - 2000000n) };
